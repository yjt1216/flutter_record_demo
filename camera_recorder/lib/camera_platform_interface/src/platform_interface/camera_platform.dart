...
abstract class CameraPlatform extends PlatformInterface {
  ...
  ///
  /// When [filePath] is provided, the platform should store the captured image
  /// at that location if supported.
  Future<XFile> takePicture(int cameraId, {String? filePath}) {
  ...
  Future<void> startVideoCapturing(VideoCaptureOptions options, {String? filePath}) {
    return startVideoRecording(options.cameraId, filePath: filePath);
  }
  ...
  /// Sets the mirror preview state for the selected camera.
  ///
  /// When [mirror] is true, the preview will be horizontally mirrored.
  Future<void> setMirrorPreview(int cameraId, bool mirror) {
    throw UnimplementedError('setMirrorPreview() is not implemented.');
  }
