...
import 'camera_platform_interface/camera_platform_interface.dart';
...
class CameraWindowsRecorder extends CameraPlatform {
  ...
  CameraWindowsRecorder({@visibleForTesting CameraApi? api})
      : _hostApi = api ?? CameraApi();

  /// Registers the Windows implementation of CameraPlatform.
  static void registerWith() {
    CameraPlatform.instance = CameraWindowsRecorder();
  }
  ...
  @override
  Future<XFile> takePicture(int cameraId, {String? filePath}) async {
    final String path = await _hostApi.takePicture(cameraId, filePath ?? '');
  ...
  Future<void> startVideoRecording(int cameraId,
      {Duration? maxVideoDuration, String? filePath}) async {
    // Ignore maxVideoDuration, as it is unimplemented and deprecated.
    return startVideoCapturing(VideoCaptureOptions(cameraId), filePath: filePath);
  }

  @override
  Future<void> startVideoCapturing(VideoCaptureOptions options, {String? filePath}) async {
    ...
    final String path = filePath ?? 'D:/VideoFiles/default_recording.mp4';
    await _hostApi.startVideoRecording(options.cameraId, path);
  }
