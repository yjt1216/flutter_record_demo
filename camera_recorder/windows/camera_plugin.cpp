...
void CameraPlugin::SetMirrorPreview(
    int64_t camera_id,
    bool mirror,
    std::function<void(std::optional<FlutterError> reply)> result) {
  auto camera = GetCameraByCameraId(camera_id);
  ...
  auto cc = camera->GetCaptureController();
  assert(cc);
  cc->SetMirrorPreviewState(mirror);
  result(std::nullopt);
}
...
void CameraPlugin::TakePicture(
  ...
  if (capture_path.empty()) {
    std::optional<std::string> generated_path = GetFilePathForPicture();
    if (!generated_path.has_value()) {
      return result(FlutterError(
          "system_error", "Failed to get capture path for picture"));
    }
